                        ...
                        if(bot_status.status =="on"):
                            ...
                            if((new_time - old_time)>35):
                                if int(u_count.auto_response)==0:
                                    ...
                                    send_state(sender_id)
                                    time.sleep(8)
                                    message_data = "Ok so the link I just sent in my first message, Fusion Cash is my favorite. They're legit and have an A+ rating with the Better Business Bureau and they're not always accepting new members, so let me know if you're able to get in. They should send you a confirmation email, be sure to click the link in there so you can get paid."
                                    send_message(sender_id, message_data)
                        else:
                        	db.session.query(User_id).update({"auto_response": 1})
                        	db.session.commit()
