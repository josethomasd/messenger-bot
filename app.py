                        ...
                        if(bot_status.status =="on"):
                            ...
                            if((new_time - old_time)>35):
                                if int(u_count.auto_response)==0:
                                    ...
                                    send_state(sender_id)
                                    time.sleep(15)
                                    message_data = "Ok I'm about to get on the road but I'll be back a litte later. In the meantime, if you get into Fusion Cash they should send a confirmation email. Be sure to click the link in there so you can get paid, then let me know you're in."
                                    send_message(sender_id, message_data)
                        else:
                        	db.session.query(User_id).update({"auto_response": 1})
                        	db.session.commit()
